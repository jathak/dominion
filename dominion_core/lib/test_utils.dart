...
class TestController extends PlayerController {
    TestController(this.name, {
        ...
        if (cardsFromHand == null) cardsFromHand = (player, context, conditions, min, max) async {
            List<Card> cards = [];
        ...
        if (cardFromSupply == null) cardFromSupply = (player, event, conditions, allowNone) async {
        ...
        if (cardsFrom == null) cardsFrom = (player, cards, question, min, max) async => cards.sublist(0, min);
        if (actionCard == null) actionCard = (player) async {
            for (Card c in player.hand.asList()) {
                if (c is ActionCard) return c;
            }
            return null;
        };
        if (treasureCards == null) treasureCards = (player) async {

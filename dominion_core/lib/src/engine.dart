...
class Player extends Object with CardSource {
    ...
    Future<bool> reactTo(EventType event, Card context) async {
        ...
        while (true) {
            List options = [];
            for (int i = 0; i < hand.length; i++) {
                if (!revealed.contains(hand[i]) && 
                        hand[i] is Reaction &&
                        hand[i].canReactTo(event)) {
                    options.add(hand[i]);
                }
            }
            if (options.length == 0) return blocked;
            options.add("None");
            var response = await controller.askQuestion(context, "Select a Reaction card to reveal.", options);
...
class Supply {
    ...
    _setup(Iterable<Card> kingdomCards, int playerCount, bool expensiveBasics) {
        _supplies = {};
        var cards = [Copper.instance, Silver.instance, Gold.instance, 
                Estate.instance, Duchy.instance, Province.instance, Curse.instance];
        if (expensiveBasics) {
